...
class ProcessOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // WhatsApp: Get dealer and retailer numbers
        $dealerMobile = User::where('id', $this->user->preferred_dealer_id)->value('mobile');
        $retailerMobile = $this->user->mobile;

        // Combine both numbers, remove null/empty ones, and ensure uniqueness
        $mobileNumbers = collect([$dealerMobile, $retailerMobile])
            ->filter() // remove null/empty
            ->unique(); // in case both numbers are same

        foreach ($mobileNumbers as $mobile) {
            $this->sendWhatsAppMessage($mobile, $order->order_no, url($pdfFilePath));
